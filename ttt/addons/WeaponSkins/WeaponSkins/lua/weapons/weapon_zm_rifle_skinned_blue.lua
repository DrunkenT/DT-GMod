...
local BASE = weapons.GetStored( "weapon_zm_rifle" )

SWEP.HoldType           		= BASE.HoldType

if CLIENT then
   SWEP.PrintName          		= BASE.PrintName .. " | Blue"
   SWEP.Slot               		= BASE.Slot
   SWEP.Icon 					= BASE.Icon
...
SWEP.Skin 						= "drunkent/tttweaponskins/v_models/skin_scout_blue"
SWEP.World 						= "drunkent/tttweaponskins/w_models/w_skin_scout_blue"
...
SWEP.Primary.ClipMax 			= BASE.Primary.ClipMax -- keep mirrored to ammo
...
function SWEP:SetZoom(state)
   ...
   if state then
      self.Owner:SetFOV(self.Owner:GetFOV() * 0.25, 0.5)
   else
