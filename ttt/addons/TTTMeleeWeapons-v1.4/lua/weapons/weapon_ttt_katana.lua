...
if CLIENT then
	...
	SWEP.Icon = "VGUI/ttt/icon_dt_katana"
...
local sound_deploy = Sound( "katana/draw.wav" )
...
function SWEP:Deploy()
	...
	self.Weapon:EmitSound( sound_deploy )
...
function SWEP:PrimaryAttack()
   ...
   if IsValid(hitEnt) or tr_main.HitWorld then
      self.Weapon:SendWeaponAnim( ACT_VM_HITCENTER )
				...
				-- Hit sound
				local sound_hit = math.random( 1, 2 )
				if (sound_hit == 1) then
					self.Weapon:EmitSound( "katana/ground_hit_1.wav" )
				else
					self.Weapon:EmitSound( "katana/ground_hit_4.wav") 
				end
				--
